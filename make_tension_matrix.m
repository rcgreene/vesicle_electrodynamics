function [output_mat, shift_mat] = make_tension_matrix(chnkr, stt_kernel_mat, smp, int_i)
...
%tmp(4:2:end, 2:2:end) = smp.n_1.v * speye(panel_res - 1, panel_res) * smp.n.u;
lob_resample = kron(speye(pan_num), tmp);
tmp(1:2:end, 1:2:end) = smp.n.v *  smp.lob_u;
tmp(2:2:end, 2:2:end) = smp.n.v *  smp.lob_u;
lob_i_resample = kron(speye(pan_num), tmp);
...
shift_mat = xs_n_1_mat * dsamp_mat * stt_kernel_mat * lob_i_resample * xsl_mat';
c_1 = shift_mat * xsl_mat * lob_resample * stt_kernel_mat * xs_mat';
c_2 = shift_mat * xsl_mat * lob_resample * stt_kernel_mat * xss_mat';
